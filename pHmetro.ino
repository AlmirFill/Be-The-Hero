...
void handleBtnLye(){
  if(digitalRead(btnLye) == HIGH)
  {
    ...
    while((millis() - timerInit < TIMER_LONG) && digitalRead(btnLye) == HIGH);

    if(millis() - timerInit >= TIMER_LONG)
    {
      lcd.clear();
      lcd.setCursor(1, 0);
      lcd.print(">> calibrar ph7 <<");
    ...
    if(millis() - timerInit < TIMER_LITLE)
    {
      ...
      dataFile = SD.open("dataBTN.txt", FILE_WRITE);
...
void handleBtnFood(){ 
  if(digitalRead(btnFood) == HIGH)
  {
    ...
    while((millis() - timerInit < TIMER_LONG) && digitalRead(btnFood) == HIGH);
...
void handleBtnSymptom(){
  if(digitalRead(btnSymptom) == HIGH)
  {
    ...
    while((millis() - timerInit < TIMER_LONG) && digitalRead(btnSymptom) == HIGH);

    if(millis() - timerInit >= TIMER_LONG)
    {
      lcd.clear();
      lcd.setCursor(1, 0);
      lcd.print(">> calibrar ph1 <<");
